...
Lo~[\hyperlink{ref:3}{3}] to continuous-time asset allocation with one
risky and one risk-free asset. One move drives the result: change the
unit of selection from investors to the individual dollars inside a
portfolio. Every dollar rides the same systematic Brownian shock, so
the Brennan-Lo dichotomy applies without modification, and its verdict
under systematic risk is randomization.

Maximizing instantaneous log-portfolio growth gives
$\lopt = \mue/\sigma^{2} + (1/\sigma)\,\dd W_{t}/\dd t$. The
deterministic part is the Kelly criterion~[\hyperlink{ref:7}{7}] and, in
the continuous-time limit, Merton's rule~[\hyperlink{ref:9}{9},\hyperlink{ref:10}{10}].
The stochastic part encodes the investor's belief about market
randomness, summarized by its correlation $\rho$ with the true Brownian
innovation, the continuous-time analog of the Brennan-Lo intelligence
parameter. Substituting $\lopt$ back into the log-portfolio dynamics
produces an intelligence correction of
$\rho - \tfrac{1}{2} \in [-\tfrac{3}{2}, +\tfrac{1}{2}]$. In plain
terms: misaligned beliefs destroy three times what aligned beliefs
create.

The same stochastic term resolves a long-standing puzzle. The
instantaneous growth-optimal rule $\lopt$ and the long-run rule
$\lkelly = \mue/\sigma^{2}$ are distinct objects, not rival answers to
one question. The randomization term is a hedge with variance
$1/(\dd t \cdot \sigma^{2})$: the expected allocation stays Kelly while
the spread around it widens as the horizon shortens. Because no
investor lives forever, that spread is always positive.
\end{quote}
...
The Brennan-Lo logic looks, at first, like a statement about
investors: the original framework describes creatures facing a binary
choice, and investors are the obvious analog. The same logic reads
just as cleanly one level down, applied to the dollars inside an
investor's portfolio. The dollars, not the investors, become the
population under selection: dollars are to the investor what investors
are to the market. Under this reframing, each dollar faces the same
systematic Brownian shock $\dd W_{t}$, the Brennan-Lo result for
systematic environments applies directly, and the growth-optimal
strategy is randomization. Each dollar draws its allocation from a
Gaussian, a biased coin flip, and the aggregate Kelly weight emerges
...
With the dynamics in hand, one question remains: which weight $\ell$
should the investor hold? The objective is the instantaneous growth of
$\ln P$. The answer, derived below, is not a number but a process.
...
The stochastic term in $\lopt$ carries the investor's belief about the
market. How good is that belief? Brennan-Lo answers with a single
number, and that number carries straight over to continuous time. The
plan for this section: state the Brennan-Lo definition, give its
continuous-time analog, then trace what intelligence does to portfolio
growth.
...
information, processing, and execution, each of them rate-limited.
Negative $\rho$ requires none of them. An investor reaches perfect
misalignment instantly, simply by holding beliefs that are exactly
backward. Misalignment, unlike intelligence, faces no speed limit.

...
\end{equation}
with $\alpha \geq 0$ and small $\varepsilon > 0$ regularizing the endpoint. The minimized cost is $\int_0^\ell L\, ds$. Both equivalent forms of $L$ --- the variational form (11) and its on-shell value $L = W(s)/v$ --- will be used: the first for variation, the second for simplification on-shell.
...
\]
Because the finishing fraction $\varphi_T$ is sampled (Section~12), every posterior draw fixes the terminal reserve and so poses a well-posed fixed-endpoint problem; a draw whose $\varphi_T$ is unreachable for the rest of $\theta$ fails to solve and is rejected by the likelihood.
...
The dynamics of Section~1 are written in path length $s$, the distance the horse's body travels. The data, and the betting question, live in the other coordinate: the track position $x \in [0, D]$, distance along the rail, with the wire at $x = D$. The two are tied by the \emph{path-stretch factor}
\begin{equation}
...
M\&A analyze races of 1300, 1900, and 2100 m. The model's $d$ is not the race distance but the horse-specific horizon at which the late-race premium begins to bite — the location around which the horse times its kick. Their Fig 10 shows $\dot V_{O_2}$ beginning to decrease about 500 m from the finish in the 1900 m race; the kick is timed to that point. The prior is centered at the 1900 m race distance with wide CV 0.5: $\mu_d = \ln(1900) = 7.550$, $\sigma_d^2 = 0.223$. The discount now integrates to the path-length endpoint $\ell = D + \Lambda(D)$ (\S\ref{sec:trip}), which ground loss lifts barely above the race distance, so $d \gtrsim \ell$ continues to seat the discount pole at the wire; a draw with $d \leq \ell$ renders the integrand non-finite there and is rejected by the likelihood.
...
Inference (Section~12) returns a posterior $\pi(\theta \mid \text{data})$ for each horse. Prediction asks the reverse question: given that posterior, what distribution does the horse's finishing time follow? The race time is the sum of the sector times,
\begin{equation}
...
\item Solve the Section~10 boundary-value problem at $\theta^{(s)}$ over $[0, \ell^{(s)}]$ (terminal reserve $\varphi_T^{(s)}$) and read the predicted sector means $\hat{\delta t}_j(\theta^{(s)})$ off the gate path lengths $s_j^{(s)}$, $j = 1, \dots, J$.
...
The variational core developed here carries its own inference machinery: the Bayesian model of Section~12, the posterior-predictive of Section~14, and the collocation solve and stack of Section~\ref{sec:numerics}. What remains for companion work is the betting layer that consumes the win-probability vector and the calibration of the model against a body of real charts.

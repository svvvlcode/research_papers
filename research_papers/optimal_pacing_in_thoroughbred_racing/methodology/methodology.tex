...
The horse runs a distance $d$. The clock runs while it does. We wish to minimize
\begin{equation}
J = \int_0^d \frac{1}{(d - s + \varepsilon)^{\alpha}} \cdot \frac{1}{v(s)}\, ds,
\end{equation}
where $s \in [0, d]$ is the distance from the start, measured in meters, and $v(s)$ is the instantaneous velocity. The factor $(d - s + \varepsilon)^{-\alpha}$, with $\alpha \geq 0$ and small $\varepsilon > 0$, weights the cost integrand more heavily near the finish, so that time lost near the wire counts more than time lost early; $\varepsilon$ regularizes the endpoint. Setting $\alpha = 0$ recovers the undiscounted race time. The state variables and physical parameters are
\begin{align*}
...
\end{align*}
The aerobic supply $\sigma(m) > 0$ is hump-shaped on $[0, m_0]$. Physiologically, when $m$ is near $m_0$ the aerobic system has not been activated and $\sigma$ is small. As $m$ decreases, oxygen uptake ramps up and $\sigma$ rises to an interior maximum. Near depletion ($m \to 0^+$), the aerobic system can no longer fully sustain peak output and $\sigma$ falls --- but it does not vanish, nor does it fall below the warm-up value: $0 < \sigma(m_0) < \sigma(0) < \max_m \sigma(m)$. The maximal force per unit mass is $f_M$ ($\mathrm{m\,s^{-2}}$).
...
\end{align}
with boundary conditions $v(0) = v_0 > 0$, $m(0) = m_0$, $m(d) = \varphi_T\, m_0$ with $0 < \varphi_T < 1$, and the state constraints $m(s) \geq 0$ and $0 \leq f(s) \leq f_M$. We drop the slope term in $\alpha(s)$ and the curvature constraint involving $c(s)$.

The first equation is Newton's law per unit mass with linear resistance. The second is the energy balance: aerobic supply $\sigma(m)$ less the mechanical power $fv$, all per unit velocity to convert from time to space. The map between $f$ and the state is one-to-one given $v$ and $m'$, and that fact is what lets us eliminate $f$ entirely. The strict time-minimizing optimum depletes the reserve fully ($m(d) \to 0$): a horse that finishes with reserve has not run the time-minimizing race. Real horses, however, finish with reserve to spare. We therefore relax the hard endpoint to a learned finishing fraction $\varphi_T = m(d)/m(0) \in (0, 1)$ and solve the Euler--Lagrange system as a fixed-endpoint boundary-value problem conditional on $\varphi_T$ --- the horse runs the time-optimal race \emph{subject to} finishing with fraction $\varphi_T$ of its reserve. We infer $\varphi_T$ for each horse (Section~11).
...
The aerobic supply curve $\sigma(m)$ has a known qualitative shape. At the start of a race, $\dot V_{O_2}$ has not yet ramped to its maximum --- the horse is warming up, and $\sigma$ is depressed. As the race progresses and $m$ depletes, $\sigma$ rises to its plateau $\sigma_{\max}$. Near the finish, when anaerobic stores can no longer compensate for oxygen deficit, $\sigma$ falls again. In short: $\sigma(m)$ is hump-shaped on $[0, m_0]$, low at both ends and peaking in the middle.

A smooth functional form that captures both knees is the product of two logistic factors:
\begin{equation}\label{eq:sigma}
\sigma(m) = \sigma_{\max}\,g_1(m)\,g_2(m),
\end{equation}
where
\[
...
\]
with $m_1 > m_2$ and $\beta_1, \beta_2 > 0$. The factor $g_1$ encodes the warm-up: it is near zero at $m \approx m_0$ (race start) and near one for $m \ll m_1$. The factor $g_2$ encodes the fall-off: it is near one for $m \gg m_2$ and near zero as $m \to 0$. Their product traces the hump.

The five parameters $\sigma_{\max}, m_1, m_2, \beta_1, \beta_2$ admit physiological interpretation. $\sigma_{\max}$ is the peak aerobic power per unit mass --- essentially $\dot V_{O_2,\max}$ in energy units. The thresholds $m_1$ and $m_2$ locate the warm-up and fall-off knees in the energy reserve. The slopes $\beta_1$ and $\beta_2$ control the sharpness of the transitions. The paper of Mercier and Aftalion uses a piecewise-linear ansatz with the same physiological content; the smooth form here is its differentiable cousin.

For the calculus of variations we need $\sigma'(m)$. Logarithmic differentiation gives, using $g_i'/g_i = \mp\beta_i(1 - g_i)/m_0$ (with the sign chosen by which factor is decreasing or increasing),
\begin{equation}\label{eq:sigmaprime}
\sigma'(m) = \frac{\sigma(m)}{m_0}\bigl[\beta_2\,w_2(m) - \beta_1\,w_1(m)\bigr],
\end{equation}
where $w_1(m) = 1 - g_1(m)$ and $w_2(m) = 1 - g_2(m)$ are the activation indicators of the warm-up and fall-off regimes, respectively. In the plateau, $w_1 \approx w_2 \approx 0$ and $\sigma'(m) \approx 0$. Near the start, $w_1$ is order one and $\sigma'(m) < 0$. Near the finish, $w_2$ is order one and $\sigma'(m) > 0$.
...
m(s) = m_0\, e^{u(s)}.
\end{equation}
The new state $u(s) \in \mathbb{R}$ is unconstrained, while $m(s) > 0$ structurally. The boundary condition $m(0) = m_0$ becomes $u(0) = 0$. The terminal reserve $m(d) = \varphi_T\, m_0$ becomes $u(d) = \ln \varphi_T$, a finite negative terminal value --- a fixed-endpoint boundary condition --- rather than the degenerate $u(d) = -\infty$ of full depletion.
...
Two equivalent expressions for $f$ follow from (8):
\begin{equation}
...
The two state equations (8) each give one expression. The $v$-dynamic, solved for $1/v$, gives the cost integrand:
\[
...
f = \frac{\sigma(m)}{v} - mu'.
\]
Substituting the second into the first eliminates $f$ from $1/v$, leaving the cost integrand as a function of $(v, v', u, u')$ alone. Multiplying by the power-law discount factor that weights time more heavily near the finish, the Lagrangian is
\begin{equation}
L(s, v, v', u, u') = W(s) \cdot \frac{v(v' + 1/\tau)}{\sigma(m(u)) - m(u)\, v\, u'}, \qquad W(s) = \frac{1}{(d - s + \varepsilon)^{\alpha}},
\end{equation}
with $\alpha \geq 0$ and small $\varepsilon > 0$ regularizing the endpoint. The minimized cost is $\int_0^d L\, ds$. Both equivalent forms of $L$ --- the variational form (11) and its on-shell value $L = W(s)/v$ --- will be used: the first for variation, the second for simplification on-shell.

The discount has logarithmic rate
\begin{equation*}
\rho(s) := \frac{W'(s)}{W(s)} = \frac{\alpha}{d - s + \varepsilon},
\end{equation*}
which is positive and grows toward the finish. The $s$-dependence breaks the autonomous structure of the Lagrangian, but the EL equations go through unchanged with $\partial L/\partial s$ now nonzero.
...
\begin{equation}
\frac{f'(s)}{f(s)} = \rho(s) - \frac{\sigma(m(s))}{v(s)^3} = \frac{\alpha}{d - s + \varepsilon} - \frac{\sigma(m(s))}{v(s)^3}.
\end{equation}
The second, derived in Appendix~\ref{app:el2}, gives a complementary first-order expression for the same growth rate:
\begin{equation}
\frac{f'(s)}{f(s)} = \rho(s) - \frac{v'(s)}{v(s)} + \frac{\sigma'(m(s))}{v(s)}.
\end{equation}
Both EL equations carry the discount through $\rho(s)$ identically; equating them yields an optimality identity in which $\rho(s)$ cancels (Section~5).

\section{The Optimality Identity}

The two EL equations constrain the same trajectory. Equating the EL1 and EL2 expressions for $f'/f$ in (12) and (13), the discount $\rho(s)$ cancels and the result is
\begin{equation}
\sigma'(m(s)) = v'(s) - \frac{\sigma(m(s))}{v(s)^2}.
\end{equation}
This is the optimality identity, derived in Appendix~\ref{app:identity}. All three terms have units of inverse seconds. The identity is a pointwise consequence of both EL conditions holding simultaneously and is independent of the discount; it serves both as an analytical lever and as a diagnostic that a numerically integrated trajectory remains on the optimal manifold.

Dividing (14) through by $v$ and substituting $\sigma(m)/v^3$ from EL1 (12) yields the rate-balance form
\begin{equation}
\frac{f'(s)}{f(s)} = \rho(s) - \frac{v'(s)}{v(s)} + \frac{\sigma'(m(s))}{v(s)},
\end{equation}
which coincides with the EL2-derived expression (13) as it must. Each term is a growth rate with respect to location, with units of inverse meters: $f'/f$ the growth rate of force, $v'/v$ the growth rate of velocity, $\sigma'(m)/v$ the growth rate of aerobic supply (since $d\sigma/ds = \sigma'(m) m'$ and $\sigma'(m)/v$ is the corresponding contribution per unit velocity), and $\rho(s)$ the growth rate of the discount. The horse runs its optimum when these rates balance pointwise. In the undiscounted limit $\rho \equiv 0$, force and velocity track each other on the aerobic plateau where $\sigma'(m) \approx 0$. With the discount active, $\rho(s)$ raises $f'/f$ everywhere, and grows toward the finish: the optimal trajectory carries more force later than in the undiscounted case.

\section{The Logit Reparameterization of the Force}

The unconstrained law of motion (12) gives $f'/f = \rho(s) - \sigma(m)/v^3$, whose sign is set by the balance between the discount premium and the aerobic drag: $f$ decays when $\sigma/v^3$ dominates and grows when $\rho$ does. We honor the empirical constraint on the force without a switching control by reparameterizing it as a convex blend between an aerobic-supply-equivalent floor $\sigma(m)/v$ and the fatigue-modulated cap $f_M \phi_f(m)$, with $\phi_f(m) \in (0, 1]$ the cap activation defined in Section~9:
\begin{equation}
f(s) = \frac{\sigma(m(s))}{v(s)} + \left(f_M \phi_f(m(s)) - \frac{\sigma(m(s))}{v(s)}\right)\sigmoid(\psi(s)), \qquad \psi(s) \in \mathbb{R}.
\end{equation}
This floors the force at $\sigma/v$ and caps it at $f_M \phi_f$, so $f(s) \in (\sigma/v, f_M \phi_f)$ structurally. The floor is the point: substituted into the reserve law (8) it gives $u'(s) = (\sigma/v - f)/m = -(f_M \phi_f - \sigma/v)\,\sigmoid(\psi)/m \leq 0$, so the reserve drains monotonically --- removing the spurious regeneration the bare logit map $f = f_M \phi_f\,\sigmoid(\psi)$ admits whenever $\sigma/v$ exceeds $f$. The trade is that the floor breaks the first EL equation where it binds: the force is then the EL-\emph{shaped} effort projected onto the monotone-feasible region rather than the strict optimum (Section~17). Translating the EL law of motion through the logit variable (Appendix~\ref{app:logit}) yields the costate rate, which we retain as the EL-shaped target:
\begin{equation}
\psi'_{\star}(s) = \frac{\rho(s) + \sigma'(m)/v - v'/v - (\phi_f'(m)/\phi_f(m))\, m\, u'}{1 - f/(f_M \phi_f(m))}.
\end{equation}
The factor $1/(1 - f/(f_M \phi_f(m)))$ stiffens the equation as $f$ approaches the (modulated) cap. The extra term in the numerator, $-(\phi_f'(m)/\phi_f(m))\, m\, u'$, accounts for the fact that the cap itself drifts downward as $m$ depletes; with $u' < 0$ and $\phi_f'(m) \geq 0$, this term is non-positive and pushes $\psi'_\star$ downward, reflecting that the available headroom is shrinking. We use the EL2 form for the EL-implied rate because it remains finite as the aerobic supply varies and exposes the discount and aerobic-sensitivity contributions explicitly.

\section{The Slew-Rate Constraint}

Empirical work (Mercier and Aftalion) imposes a separate slew-rate constraint $u_-/v \leq f' \leq u_+/v$ via bang-bang controls on $df/dt$, with $u_-, u_+ > 0$ asymmetric physiological limits on the rate of force buildup and release. We honor it without switching by bounding $\psi'$, which co-varies with $f'$. Differentiating the blend (16) along the trajectory, with $S = \sigmoid(\psi)$ and the cap written $C = f_M \phi_f(m)$,
...
\section{Fatigue-Activation Functions}
...
\end{cases}
\end{equation}
with $m = m_0 e^{u(s)}$ throughout, $f$ the blend (16), the half-widths $A, B$ of \eqref{eq:slew-band}, and boundary conditions
\[
v(0) = v_0, \qquad u(0) = 0 \quad \text{(gate)}, \qquad u(d) = \ln \varphi_T \quad \text{(wire)}.
\]
The optimal velocity is the state $v(s)$ directly. The optimal reserve is recovered as $m(s) = m_0 e^{u(s)}$, with $m(s) > 0$ along the trajectory by construction and, by the floor, draining monotonically. The optimal propulsive force is recovered from the blend (16), with $f(s) \in (\sigma/v, f_M \phi_f)$ by construction; the gate force $f(0)$ is read off the selected $\psi(0)$, not supplied as an input.

\paragraph{Two-point boundary-value problem.} The gate costate $\psi(0)$ is the one free boundary unknown, matched to the one terminal condition $u(d) = \ln \varphi_T$ by a shooting or collocation solve. The gate force then falls out as a derived quantity,
\[
f_0 = f_M\, \phi_f(m_0)\, \sigmoid(\psi(0)).
\]
Because the finishing fraction $\varphi_T$ is sampled (Section~11), every posterior draw fixes the terminal reserve and so poses a well-posed fixed-endpoint problem; a draw whose $\varphi_T$ is unreachable for the rest of $\theta$ fails to solve and is rejected by the likelihood.

The optimality identity (14) is a consequence of the first-order system on-shell and is not used for the solve; it remains useful for analytical inspection and as a numerical diagnostic.

\paragraph{Race mechanics.} EL2 (13) writes the growth rate of force with respect to location as
\[
\frac{f'}{f} = \rho(s) + \frac{\sigma'(m)}{v} - \frac{v'}{v},
\]
the late-race premium plus the marginal aerobic sensitivity, less the growth rate of velocity. The discount enters as
\[
\rho(s) = \frac{\alpha}{d - s + \varepsilon}.
\]
Because $\sigma$ is hump-shaped on $[0, m_0]$ and the horse traverses it from right to left as $m$ decreases,
\[
...
From the gate, $m$ is near $m_0$. Both fatigue activations are saturated near unity; the cap is the unfatigued $f_M$ and the upper slew rate is the unfatigued $b$. $\sigma'(m)$ is large and negative, but $\rho$ is small and the aerobic term has not yet bitten through specific kinetic energy; what matters dynamically is that EL1 calls for force buildup against a small $v$. the build slew saturates at the unfatigued rate $b$ and $f$ rises along the smooth analog of a bang-bang ramp.

In the body of the race, $\sigma(m)$ traverses its interior maximum, where $\sigma'(m) \approx 0$. The EL2 form reduces to
\[
\frac{f'}{f} \approx \rho(s) - \frac{v'}{v}.
\]
$m$ has fallen but typically remains above $m_f^*$ and $m_b^*$, so $\phi_f, \phi_b \approx 1$ and the caps are still effectively the unfatigued ones. Force and velocity track each other up to the discount premium. With $\rho$ still modest mid-race, $f$ decays slightly slower than $v$.

Near the finish, three effects compound. The premium $\rho(s)$ grows without bound. $\sigma'(m)$ turns positive as $\sigma$ falls off its peak toward $\sigma(0)$. And the fatigue activations begin to bite: $m$ crosses $m_f^*$ and $m_b^*$, $\phi_f$ pulls the cap down toward $f_M \phi_{f,\min}$, and $\phi_b$ pulls the upper slew rate down toward $b\, \phi_{b,\min}$. The first two effects raise the EL-implied $f'/f$; the third caps how much of that rise the horse can execute. A finishing kick emerges, but bounded twice over: by the modulated cap on $f$ itself, and by the modulated rate at which $f$ can climb. The kick is real but smaller than the unfatigued model would predict, and arrives with $f$ unable to reach the unfatigued $f_M$. The undiscounted, unfatigued limit ($\alpha = 0$, $\phi_f = \phi_b = 1$) recovers monotonic decay through the wire.

\section{Bayesian Inference}

The forward system in Section~10 is deterministic: given a parameter vector $\theta$, the integrator produces a velocity trajectory $\hat v(s; \theta)$ and, derivatively, predicted sector times $\hat{\delta t}_j(\theta) = \int_{s_{j-1}}^{s_j} ds/\hat v(s; \theta)$. Real races deliver noisy sector times, and the parameter vector is what we wish to learn. We approach this with Bayesian inference: for each horse, given its interval race data, we recover a posterior distribution over $\theta$ that combines a physiologically-informed prior with the likelihood of the observed sector times under the model.
...
\paragraph{Reparameterization for inference.} Each parameter is mapped to an unconstrained scale on which inference is performed. Strictly positive parameters carry a $\ln$ transform. Parameters bounded in $(0, c)$ for some $c$ carry a logit-with-scale transform $\ln(c x / (c - x))$; the finishing fraction $\varphi_T \in (0, 1)$ is the $c = 1$ case, $\ln(x/(1-x))$. The gate force $f_0$ is no longer sampled --- it is derived from the boundary-value solve (Section~10) --- and $\varphi_T$ takes its place in the parameter vector, entering as the terminal condition $u(d) = \ln \varphi_T$ with the Beta prior below. Three reserve quantities $m_0, m_1, m_2$ are reparameterized into independent latents $z_2, z_{12}, z_{01}$ that enforce $0 < m_2 < m_1 < m_0$ by construction:
\[
m_2 = e^{z_2}, \qquad m_1 = m_2(1 + e^{z_{12}}), \qquad m_0 = m_1(1 + e^{z_{01}}).
\]
...
\subsection*{Discount horizon $d$}
M\&A analyze races of 1300, 1900, and 2100 m. The model's $d$ is not the race distance but the horse-specific horizon at which the late-race premium begins to bite — the location around which the horse times its kick. Their Fig 10 shows $\dot V_{O_2}$ beginning to decrease about 500 m from the finish in the 1900 m race; the kick is timed to that point. The prior is centered at the 1900 m race distance with wide CV 0.5: $\mu_d = \ln(1900) = 7.550$, $\sigma_d^2 = 0.223$.

\subsection*{Endpoint regularizer $\varepsilon$}
The factor $\varepsilon$ prevents $\rho(s) = \alpha/(d - s + \varepsilon)$ from diverging at $s = d$. Its value sets the scale over which the discount integrand peaks. M\&A identify the final sprint as occupying roughly the last 50–100 m. Centered at $\varepsilon = 50$ m with CV 0.5: $\mu_\varepsilon = \ln(50) = 3.912$, $\sigma_\varepsilon^2 = 0.223$.

\subsection*{Power-law exponent $\alpha$}
No published value exists for the exponent of the late-race premium. Centered at $\alpha = 1$ with CV 0.5, admitting exponents from roughly 0.4 to 2.5 at one prior SD: $\mu_\alpha = 0$, $\sigma_\alpha^2 = 0.223$.
...
\subsection*{Reserve latents $z_2, z_{12}, z_{01}$}
M\&A Fig 10 shows the residual reserve at the $\dot V_{O_2}$ fall-off (sprint launch) at approximately one-third of $e^0$ for the longer races. With $m_0 \approx 3000$ J kg$^{-1}$, $m_2 \approx 1000$, hence $\mu_{z_2} = \ln(1000) = 6.908$. The first slope change in Fig 10 marks reaching $\dot V_{O_2,\max}$, where warm-up has fully activated; cross-horse average of $m_1$ is approximately $0.74 m_0 \approx 2220$, yielding gap $m_1/m_2 - 1 \approx 1.22$, hence $\mu_{z_{12}} = \ln(1.22) = 0.199$. With $m_1 \approx 2220$ and $m_0 \approx 3000$, gap $m_0/m_1 - 1 \approx 0.351$, hence $\mu_{z_{01}} = \ln(0.351) = -1.047$. All three carry CV 0.3: $\sigma^2 = 0.0862$.

\subsection*{Knee steepnesses $\beta_1, \beta_2$}
M\&A use a piecewise-linear $\sigma$ profile with sharp corners. A smooth logistic with $\beta = 10$ traverses each transition over roughly $0.4 m_0$, matching the visual sharpness of their Figs 4a, 6a, 9a. CV 0.5: $\mu_{\beta_1} = \mu_{\beta_2} = \ln(10) = 2.303$, $\sigma^2 = 0.223$.
...
No published value. Same scale as the $\beta$'s, $\kappa_f \approx 10$. CV 0.5: $\mu_{\kappa_f} = 2.303$, $\sigma^2 = 0.223$.
...
\midrule
\endhead
$d$ & Discount horizon & m & $(0, \infty)$ & $\ln$ & $\mathcal{N}$ & 7.550 & 0.223 \\
$\varepsilon$ & Endpoint regularizer & m & $(0, \infty)$ & $\ln$ & $\mathcal{N}$ & 3.912 & 0.223 \\
$\alpha$ & Discount exponent & --- & $[0, \infty)$ & $\ln$ & $\mathcal{N}$ & 0 & 0.223 \\
...
$z_{12}$ & $m_1 = m_2(1+e^{z_{12}})$ & --- & $\mathbb{R}$ & id & $\mathcal{N}$ & 0.199 & 0.0862 \\
$z_{01}$ & $m_0 = m_1(1+e^{z_{01}})$ & --- & $\mathbb{R}$ & id & $\mathcal{N}$ & $-1.047$ & 0.0862 \\
$\beta_1$ & Warm-up steepness & --- & $(0, \infty)$ & $\ln$ & $\mathcal{N}$ & 2.303 & 0.223 \\
...
\section{The Posterior-Predictive Race Time}

Inference (Section~11) returns a posterior $\pi(\theta \mid \text{data})$ for each horse. Prediction asks the reverse question: given that posterior, what distribution does the horse's finishing time follow? The race time is the sum of the sector times,
\begin{equation}
...
\item Solve the Section~10 boundary-value problem at $\theta^{(s)}$ (terminal reserve $\varphi_T^{(s)}$) to obtain the predicted sector means $\hat{\delta t}_j(\theta^{(s)})$, $j = 1, \dots, J$.
...
\paragraph{Hermite--Simpson collocation for the boundary-value problem.} The forward system of Section~10 is a two-point boundary-value problem: $v(0) = v_0$ and $u(0) = 0$ at the gate, $u(d) = \ln \varphi_T$ at the wire, with the gate costate $\psi(0)$ left free. The Euler--Lagrange system is a Hamiltonian saddle, and the costate $\psi$ is anti-stable under forward integration --- a small error in a guessed $\psi(0)$ grows exponentially over $[0, d]$, and reversing the direction of integration only moves the blow-up onto the states $(v, u)$. So rather than shoot from one end, we discretize the whole trajectory and solve it at once. Lay down a mesh $0 = s_0 < s_1 < \cdots < s_N = d$ with steps $h_k = s_{k+1} - s_k$, and carry the nodal states $Y_k \approx y(s_k)$, $y = (v, u, \psi)$, as the unknowns. On each subinterval the cubic Hermite interpolant through the endpoint values and their slopes $F_k = F(s_k, Y_k)$ has midpoint $y_{\mathrm{mid}} = \tfrac{1}{2}(Y_k + Y_{k+1}) + \tfrac{h_k}{8}(F_k - F_{k+1})$; requiring the differential equation to hold at that midpoint is Simpson's rule, and the residual it leaves is the defect
...
Two of these moves trade strict optimality for numerical robustness, and we state the trade plainly. The force floor breaks the first Euler--Lagrange equation wherever it binds, so the trajectory is the EL-shaped effort projected onto the monotone-feasible region, not the strict time optimum; and the smooth slew saturation abandons optimality at the band boundary by construction. In exchange we get a force positive above the floor, a reserve that depletes monotonically, and a boundary-value problem whose residual is smooth and well-conditioned --- the properties that let the solve sit inside the sampler. The undiscounted, unfatigued, floor-free limit recovers the strict variational optimum; the working model is a deliberate, bounded departure from it.

The optimality identity (14), which appears only after equating two derivations of $f'$, is the algebraic content of optimality stripped of dynamical specifics. Read pointwise, it equates the marginal aerobic sensitivity to a kinematic combination: the acceleration plus the negative ratio of aerobic supply to specific kinetic energy. The horse runs its optimum when these are in balance.

The variational core developed here carries its own inference machinery: the Bayesian model of Section~11, the posterior-predictive of Section~13, and the collocation solve and stack of Section~\ref{sec:numerics}. What remains for companion work is the betting layer that consumes the win-probability vector and the calibration of the model against a body of real charts.

\appendix
\section{Derivations}

The Lagrangian (11) and its log form
\begin{equation}
\ln L = \ln v + \ln(v' + 1/\tau) - \ln(\sigma(m) - m\, v\, u')
\end{equation}
underlie all derivations below. Using $m(u) = m_0 e^u$ so that $m'(u) = m$, and $L'/L \equiv (d/ds) \ln L = \rho(s) - v'/v$ on-shell (with $\rho(s) = W'(s)/W(s) = \alpha/(d-s+\varepsilon)$), the partial derivatives of $\ln L$ are
\begin{align*}
...
\end{align*}
The discount factor $W(s)$ is independent of $v, v', u, u'$, so it drops out of all four logarithmic partials; it enters only through $L'/L$ and through the on-shell value $L = W(s)/v$. Each EL equation $\partial L/\partial q = (d/ds)\, \partial L/\partial q'$ for $q \in \{v, u\}$ is divided by $L$ on both sides and read as a relation among logarithmic derivatives.
...
\]
On-shell, $L = W(s)/v$, so $L/(v' + 1/\tau) = W/[v(v' + 1/\tau)] = W/f$ by the $v$-dynamics expression $f = v(v' + 1/\tau)$. Therefore
\[
\frac{d}{ds}\ln\left(\frac{W}{f}\right) = \frac{W'}{W} - \frac{f'}{f} = \rho(s) - \frac{f'}{f}.
\]
Equating with $\sigma(m)/v^3$ and solving for $f'/f$ yields (12).

\subsection{The second EL equation}\label{app:el2}

The partial derivatives of $\ln L$ in $u$ and $u'$ are independent of $W(s)$ since $W$ has no $u$ or $u'$ dependence. The EL equation $\partial L/\partial u = (d/ds)\, \partial L/\partial u'$, written in terms of $L$ times the logarithmic partials, is
\[
...
\]
On-shell, $L = W/v$, so $Lmv/(\sigma - mvu') = Wm/(\sigma - mvu')$. Applying the on-shell identity $\sigma - mvu' = v^2(v' + 1/\tau) = vf$ to the denominator,
\[
\frac{Lmv}{\sigma - mvu'} = \frac{Wm}{vf}.
\]
Therefore
\[
\frac{d}{ds}\ln\!\left(\frac{Wm}{vf}\right) = \frac{W'}{W} + \frac{m'}{m} - \frac{v'}{v} - \frac{f'}{f} = \rho(s) + u' - \frac{v'}{v} - \frac{f'}{f},
\]
using $m'/m = u'$. Substituting back,
\[
-(\sigma'(m) - vu') = v\left[\rho(s) + u' - \frac{v'}{v} - \frac{f'}{f}\right],
\]
the $vu'$ terms cancel, and solving for $f'/f$ yields the EL2 expression for the law of motion of the propulsive force:
\begin{equation}
\frac{f'(s)}{f(s)} = \rho(s) - \frac{v'(s)}{v(s)} + \frac{\sigma'(m(s))}{v(s)}.
\end{equation}
Equating this with EL1 (12) gives
\[
\rho(s) - \frac{\sigma(m)}{v^3} = \rho(s) - \frac{v'}{v} + \frac{\sigma'(m)}{v},
\]
in which $\rho(s)$ cancels, leaving the optimality identity (14) unchanged by the discount.

\subsection{The optimality identity}\label{app:identity}

Equating the EL1 (12) and EL2 (13) expressions for $f'/f$,
\[
\rho(s) - \frac{\sigma(m)}{v^3} = \rho(s) - \frac{v'}{v} + \frac{\sigma'(m)}{v},
\]
the discount $\rho(s)$ cancels. Multiplying through by $v$ and rearranging yields the optimality identity (11). The identity is therefore independent of the discount: $\rho(s)$ enters both EL equations identically through $L'/L$ and drops out of their joint content.

\subsection{The logit reparameterization and slew bound}\label{app:logit}

The costate rate (17) comes from the logit map $f = f_M \phi_f(m)\,\sigmoid(\psi)$, the change of variables that renders the force reparameterization exact. Differentiating along the trajectory, with $\sigmoid'(\psi) = \sigmoid(\psi)(1 - \sigmoid(\psi))$,
\[
\frac{f'}{f} = \bigl(1 - \sigmoid(\psi)\bigr)\, \psi' = \left(1 - \frac{f}{f_M \phi_f(m)}\right)\psi'.
\]
Setting this equal to the EL2 expression for $f'/f$ from (13) and solving for $\psi'$ yields (17). The floored blend (16) keeps this $\psi'_\star$ as the EL-\emph{shaped} target rate --- the costate retains its EL2 shape and headroom factor --- which the saturation \eqref{eq:slew-squash} then bounds.
